...
from .models import Group, Domain, Dns, Whois, Geoip, SslLookup, CmsLookup, Log
...
from .serializers import CompanyClientSerializer, CompanyTeamSerializer, CompanyDomainSerializer, CompanyGroupSerializer, CompanyDNSSerializer, CompanyWhoisSerializer, CompanyGeoIpSerializer, CompanySSLSerializer, LogSerializer
from rest_framework.permissions import IsAuthenticated

from rest_framework.parsers import MultiPartParser, FormParser
from .dns_records import lookup_dns_records
...
from .whois_records import save_whois_data
from .geoip_records import save_geoip_data
...
class CompanyClientView(APIView):
    ...
    def get(self, request, format=None):
        clients = User.objects.filter(role="Client", company=request.user.id)
...
class CompanyTeamView(APIView):
    ...
    def get(self, request, format=None):
        teams = User.objects.filter(role="Team", company=request.user.id)
...
class CompanyGroupView(APIView):
    ...
    def get(self, request, format=None):
        groups = Group.objects.filter(company=request.user.id)
...
class CompanyDomainView(APIView):
    ...
    def get(self, request, format=None):
        domains = Domain.objects.filter(company=request.user.id)
...
class CompanyDNSView(APIView):
    ...
    def get(self, request, format=None):
        dns = Dns.objects.filter(company=request.user.id)
    ...
    def post(self, request):
        ...
        # print(domain)
        records = lookup_dns_records(request.user.id, domain)
...
class CompanyWhoisView(APIView):
    ...
    def get(self, request):
        whoisData = Whois.objects.filter(company=request.user.id)
...
class CompanyGeoIpView(APIView):
    ...
    def get(self, request):
        geoip = Geoip.objects.filter(company=request.user.id)
...
class CompanySSLView(APIView):
    ...
    def get(self, request):
        ssl = SslLookup.objects.filter(company=request.user.id)
